...
export async function sendEmail({
  ...
  text,
  html,
  from = "procurement-ai@example.com",
}: EmailParams): Promise<boolean> {
  try {
    // If SendGrid is not configured, simulate sending email
    if (!mailService) {
...
export async function sendNegotiationInvitation({
  ...
  const subject = `Invitation to Negotiation: ${categoryName}`;
  
  const text = `
Dear ${supplierName},

You have been invited to participate in an AI-powered negotiation for the ${categoryName} category.
...
<head>
  <style>
    ...
    .button { display: inline-block; padding: 10px 20px; background-color: #4a6cf7; color: #ffffff; text-decoration: none; border-radius: 4px; }
...
<body>
  <div class="container">
    ...
    <p>You have been invited to participate in an AI-powered negotiation for the <strong>${categoryName}</strong> category.</p>
    <p>Please click the button below to join the negotiation:</p>
